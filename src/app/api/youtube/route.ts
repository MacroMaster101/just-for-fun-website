...
export async function GET() {
  try {
    ...
    console.error("YouTube API proxy error:", cleanMessage);
    return await fallbackResponse(cleanMessage);
  }
}
...
async function getChannel() {
  ...
  if (channelById?.items?.[0]) {
    return channelById.items[0];
  }
  ...
  if (!channel) {
    throw new Error(`No YouTube channel found for ${CHANNEL_ID || CHANNEL_HANDLE}.`);
  }
...
async function getLatestUploadVideos(uploadsPlaylistId: string) {
  const playlistItems = await fetchYouTube<PlaylistItemsResponse>("playlistItems", {
...
async function getRssFallbackVideos() {
...
function extractTag(entry: string, tag: string) {
  ...
  const match = new RegExp(`<${escapedTag}>([\\s\\S]*?)<\\/${escapedTag}>`).exec(
    entry
  );
  return match?.[1] || "";
}
...
async function fallbackResponse(message: string) {
...
function youtubeJson(payload: unknown) {
